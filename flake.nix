  ...
  outputs =
    ...
    let
      overlay = (final: prev:
        ...
        in {
          medik-pyk = poetry2nix.mkPoetryApplication {
            python = nixpkgs-pyk.python310;
            projectDir = ./medik-pyk;
          ...
          medik = prev.stdenv.mkDerivation {
            pname = "medik";
            ...
            installPhase = ''
              ...
              makeWrapper ${final.medik-pyk}/bin/kmir $out/bin/kmir \
                --set MEDIK_LLVM_DIR "$out/lib/llvm" \
                --set MEDIK_HASKELL_DIR "$out/lib/haskell" \
                --prefix PATH : ${
          ...
          medik-test = prev.stdenv.mkDerivation {
            pname = "medik-test";
            version = self.rev or "dirty";

            src = ./.;

            buildInputs = [ final.medik prev.which prev.git ];
